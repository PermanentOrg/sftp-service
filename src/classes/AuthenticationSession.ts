...
export class AuthenticationSession {
	...
	private processPasswordResponse([password]: string[]): void {
		this.fusionAuthClient
				...
				switch (clientResponse.statusCode) {
					...
					case FusionAuthStatusCode.SuccessButUnregisteredInApp.valueOf(): {
						...
						this.registerUserInApp(userId)
							...
							.catch((error: unknown) => {
								logger.warn(
									"Error during registration and authentication:",
									error,
								);
								this.authContext.reject();
							});
			...
			.catch((error: unknown) => {
				...
				logger.warn(
					`Unexpected exception with FusionAuth password login: ${message}`,
				);
				this.authContext.reject();
			});
	}
	...
	private async registerUserInApp(userId: string): Promise<void> {
		try {
			...
			logger.warn(
				"Error during user registration after authentication:",
				error,
			);
		}
	}
	...
	private processTwoFactorMethodResponse([methodIndex]: string[]): void {
		...
		const selection = Number.parseInt(methodIndex, 10);
		if (
	...
	private processTwoFactorCodeResponse([twoFactorCode]: string[]): void {
		this.fusionAuthClient
			...
			.catch((error: unknown) => {
				...
				logger.warn(
					`Unexpected exception with FusionAuth 2FA login: ${message}`,
				);
				this.authContext.reject();
			});
	}
