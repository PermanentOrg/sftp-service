...
export class TemporaryFileManager {
  ...
  private readonly temporaryFileCleanupTimeouts = new Map<string, NodeJS.Timeout>();

  private static async verifyPathExistsOnDisk(localPath: string): Promise<boolean> {
  ...
  public async getTemporaryFile(virtualPath: string): Promise<TemporaryFile> {
    ...
    if (temporaryFile === undefined) {
      throw new MissingTemporaryFileError('Attempted to access a temporary file that does not exist in memory.');
    }
    if (!(await TemporaryFileManager.verifyPathExistsOnDisk(temporaryFile.name))) {
      throw new MissingTemporaryFileError('Attempted to access a temporary file that does not exist on disk.');
    }
  ...
  public async createTemporaryFile(virtualPath: string): Promise<TemporaryFile> {
      ...
      () => {
        ...
        this.deleteTemporaryFile(virtualPath)
          .catch((err:unknown) => {
            ...
            logger.info(err);
            logger.info(`Unable to delete temporary file associated with ${virtualPath}`);
          });
      },
      86400000, // 24 hours
    );
