...
describe("getOriginalFileForArchiveRecord", () => {
	it("should return the original file from the archive record", () => {
	...
	it("should find the original file among multiple files", () => {
	...
	it("should return the first original file when multiple exist", () => {
	...
	it("should throw an error when no original file exists", () => {
		const convertedFile = createTestFile({
			derivativeType: DerivativeType.Converted,
		});
		const archiveRecord = createTestArchiveRecord({
	...
	it("should throw an error when files array is empty", () => {
		const archiveRecord = createTestArchiveRecord({
			id: 99,
			files: [],
		});
